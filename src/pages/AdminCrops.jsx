import {
  PlusCircle, Image, Clock, Droplet, ChevronDown, Sprout, Trees
} from "lucide-react";
...
const AdminCrops = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.error(err?.data?.error || "Failed to create crop");
    }
  };
    ...
    <div className="max-w-3xl mx-auto my-6 p-5 bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="bg-amber-50/50 p-4 rounded-lg border border-amber-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs font-medium text-gray-600">Growth Period</label>
              <input
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-gray-600">Water Requirements</label>
              <input
        ...
        <div className="bg-emerald-50/50 p-4 rounded-lg border border-emerald-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs font-medium text-gray-600">Soil Requirements*</label>
              <textarea
              ...
              />
              {errors.soilRequirements && <p className="text-red-500 text-xs">{errors.soilRequirements.message}</p>}
            </div>
            <div className="space-y-1">
              <label className="text-xs font-medium text-gray-600">Fertilizers</label>
              <textarea
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-gradient-to-r from-green-600 to-emerald-600 text-white text-sm font-semibold py-2 rounded-lg hover:shadow-md transition-all flex items-center justify-center disabled:opacity-50"
        >
